    ...
    case 'trust_badge':
      ...
      return (
        <div className="flex items-center justify-center gap-2 py-2 text-xs text-gray-400">
          ...
          {cfg.trust_badge_text || 'Pago seguro contraentrega'}
        </div>
      );

    case 'shipping_info':
      if (!cfg.show_shipping_method) return null;
      return (
        <div className="flex items-center justify-center gap-2 py-1 text-xs text-gray-500">
          ...
          {cfg.shipping_text || 'Envio gratis'}
        </div>
      );
    ...
    case 'payment_method':
      return (
        <div className="flex items-center justify-center gap-2 py-1 text-xs text-gray-500">
          ...
          {cfg.payment_method_text || 'Pago contraentrega'}
        </div>
      );

    case 'submit_button':
      // Non-sticky inline submit button (rendered only if sticky is disabled)
      ...
      return (
        <div className="px-0">
          <button
            ...
            className="w-full py-4 font-bold shadow-lg transition-all hover:opacity-90 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {submitting ? 'Procesando...' : (cfg.cta_text || 'Completar pedido').replace('{order_total}', totalPriceFormatted)}
          </button>
        </div>
      );
